...
export const Dashboard = ({ 
  ...
  const [playerStats, setPlayerStats] = useState<PlayerStats>({
    ...
    streak: studentData?.dailyStreak ?? studentData?.daily_streak ?? 0,
    timeSpent: studentData?.totalTimeMinutes ?? studentData?.total_time_minutes ?? 0,
    achievements: [],
    completedGames: []
  });
    ...
    (async () => {
      ...
      try {
        ...
        if (response.success && response.data) {
          setPlayerStats(prev => ({
            ...prev,
            points: (response.data.gamesPlayed || 0) > 0 ? (response.data.currentScore || 0) : 0,
            gamesPlayed: response.data.gamesPlayed || 0,
            streak: response.data.dailyStreak || 0,
            timeSpent: response.data.totalTimeMinutes || 0,
          }));
        }
      } catch (e) {
  ...
  const handleGameComplete = async (gameId: string, score: number, total: number) => {
    ...
    try {
      if (studentData?.id) {
        const profileResponse = await studentAPI.getProfile(studentData.id);
        if (profileResponse.success && profileResponse.data) {
          const dbScore = profileResponse.data.currentScore || 0;
          const updateResponse = await studentAPI.updateProgress(
            studentData.id,
            nextGamesPlayed,
            dbScore + pointsEarned
          );
          if (updateResponse.success && updateResponse.data) {
            setPlayerStats(prev => ({
              ...prev,
              points: updateResponse.data.currentScore || dbScore + pointsEarned,
              gamesPlayed: updateResponse.data.gamesPlayed || nextGamesPlayed
            }));
          }
        }
      }
  ...
  useEffect(() => {
    ...
    const flushActivity = async (playedToday: boolean) => {
      ...
      try {
        await studentAPI.updateActivity(studentData.id, sessionMinutes, playedToday);
        sessionMinutes = 0;
      } catch (e) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await studentAPI.updateActivity(studentData.id, 0, true);
