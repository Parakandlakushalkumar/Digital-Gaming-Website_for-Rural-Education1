...
export const AssignmentManager = ({ students, subjects, currentLanguage, teacherId }: AssignmentManagerProps) => {
  ...
  const loadAssignments = async () => {
    try {
      const response = await assignmentAPI.getAssignments(teacherId);
  ...
  const loadSubmissions = async () => {
    try {
      const response = await teacherAPI.getSubmissions(teacherId);
  ...
  const handleCreateAssignment = async () => {
    ...
    try {
      if (newAssignment.grade) {
        await assignmentAPI.createAssignmentByClass({
          title: newAssignment.title,
          description: newAssignment.description || "",
          subject: newAssignment.subject,
          dueDate: newAssignment.dueDate,
          teacherId,
          grade: parseInt(newAssignment.grade, 10)
        });
      } else {
        await assignmentAPI.createAssignment({
          ...
          dueDate: newAssignment.dueDate,
          teacherId,
          studentIds: newAssignment.assignedStudents
        });
      }
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 text-sm text-muted-foreground">
                      ...
                      <span className="flex items-center gap-1">
                        ...
                        {students.filter(s => (s.teacherId || s.teacher_allocated) === teacherId).length} {t.students}
                      </span>
                    </div>
                  </div>
