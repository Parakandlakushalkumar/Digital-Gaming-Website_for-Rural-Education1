...
export const ClassAnalytics = ({ students, currentLanguage }: ClassAnalyticsProps) => {
  ...
  const calculateSubjectPerformance = () => {
    ...
    const subjectData = subjects.map((subject, index) => {
      ...
      const baseScore = students.reduce((sum, student) => sum + getCurrentScore(student), 0) / students.length;
  ...
  const calculateProgressDistribution = () => {
    ...
    const completed = students.filter(student => getGamesPlayed(student) >= 4).length;
    const inProgress = students.filter(student => {
      const gamesPlayed = getGamesPlayed(student);
      return gamesPlayed > 1 && gamesPlayed < 4;
    }).length;
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="w-16 bg-muted rounded-full h-2">
                    <div 
                      ...
                      style={{ width: `${students.length ? (item.missed / students.length) * 100 : 0}%` }}
                    />
                  </div>
                </div>
              </div>
